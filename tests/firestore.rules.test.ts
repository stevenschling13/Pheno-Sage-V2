...
describe('Firestore Security Rules', () => {
  ...
  describe('Grows', () => {
    it('allows user to create a valid grow', async () => {
      const db = testEnv.authenticatedContext('alice').firestore();
      await assertSucceeds(setDoc(doc(db, 'grows', 'grow1'), {
        ownerId: 'alice',
        name: 'My Grow',
        stage: 'Vegetative',
        medium: 'Soil',
    ...
    it('rejects grow create with extra fields', async () => {
      const db = testEnv.authenticatedContext('alice').firestore();
      await assertFails(setDoc(doc(db, 'grows', 'grow1'), {
        ownerId: 'alice',
        name: 'My Grow',
        stage: 'Vegetative',
        medium: 'Soil',
    ...
    it('rejects grow update ownerId mutation', async () => {
      ...
      await assertSucceeds(setDoc(growRef, {
        ownerId: 'alice',
        name: 'My Grow',
        stage: 'Vegetative',
        medium: 'Soil',
      ...
      await assertFails(updateDoc(growRef, {
        ownerId: 'bob',
        updatedAt: serverTimestamp()
      }));
    });
  ...
  describe('Plants', () => {
    ...
    it('allows user to create a valid plant under their own grow', async () => {
      const db = testEnv.authenticatedContext('alice').firestore();
      await assertSucceeds(setDoc(doc(db, 'plants', 'plant1'), {
        ownerId: 'alice',
        growId: 'aliceGrow',
        name: 'Plant 1',
        strain: 'OG Kush',
    ...
    it('rejects plant create pointing to another users grow', async () => {
      const db = testEnv.authenticatedContext('alice').firestore();
      await assertFails(setDoc(doc(db, 'plants', 'plant1'), {
        ownerId: 'alice',
        growId: 'bobGrow',
        name: 'Plant 1',
        strain: 'OG Kush',
    ...
    it('rejects plant update ownerId mutation', async () => {
      ...
      await assertSucceeds(setDoc(plantRef, {
        ownerId: 'alice',
        growId: 'aliceGrow',
        name: 'Plant 1',
        strain: 'OG Kush',
      ...
      await assertFails(updateDoc(plantRef, {
        ownerId: 'bob',
        updatedAt: serverTimestamp()
      }));
    });
  ...
  describe('Media Assets', () => {
    ...
    it('denies create with extra ID field', async () => {
      const db = testEnv.authenticatedContext('alice').firestore();
      await assertFails(setDoc(doc(db, 'media_assets', 'media1'), { ...validMediaAsset, id: 'media1' }));
    });

    it('denies create if owner is wrong', async () => {
      const db = testEnv.authenticatedContext('alice').firestore();
      await assertFails(setDoc(doc(db, 'media_assets', 'media1'), { ...validMediaAsset, ownerId: 'bob', storagePath: 'users/bob/grows/aliceGrow/plants/alicePlant/media/media1' }));
    });

    it('denies create if grow belongs to someone else', async () => {
      const db = testEnv.authenticatedContext('alice').firestore();
      await assertFails(setDoc(doc(db, 'media_assets', 'media1'), { ...validMediaAsset, growId: 'bobGrow', storagePath: 'users/alice/grows/bobGrow/plants/alicePlant/media/media1' }));
    });

    it('denies create if plant belongs to someone else', async () => {
      const db = testEnv.authenticatedContext('alice').firestore();
      await assertFails(setDoc(doc(db, 'media_assets', 'media1'), { ...validMediaAsset, plantId: 'bobPlant', storagePath: 'users/alice/grows/aliceGrow/plants/bobPlant/media/media1' }));
    });
    ...
    it('allows update when only uploadStatus and updatedAt change', async () => {
      ...
      await assertSucceeds(updateDoc(mediaRef, { uploadStatus: 'uploaded', updatedAt: serverTimestamp() }));
    });
  ...
  describe('Future Controlled Collections', () => {
    it('denies client writes to analysis, findings, notifications, prompts, chat', async () => {
      ...
      await assertFails(setDoc(doc(db, 'chat_threads/thread1/messages', 'msg1'), { ownerId: 'alice' }));
    });
  });
});
