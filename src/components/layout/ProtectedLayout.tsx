import { LayoutDashboard, Leaf, Sprout, LogOut, Camera, MessageSquare, AlertTriangle, Loader2 } from 'lucide-react';
...
export default function ProtectedLayout() {
  ...
  return (
    <div className="flex h-[100dvh] flex-col bg-brand-bg font-sans text-zinc-300 antialiased overflow-hidden selection:bg-status-optimal selection:text-white">
      {/* Global Header */}
      <header className="flex h-12 shrink-0 items-center justify-between border-b border-brand-border bg-brand-surface px-4 md:px-6 z-10 relative">
      ...
      <div className="flex flex-1 overflow-hidden relative">
        ...
        <aside className="hidden md:flex w-56 flex-col border-r border-brand-border bg-brand-surface/50 p-3 shrink-0">
        ...
        {/* Main Content Area */}
        <main className="flex-1 overflow-auto pb-16 md:pb-0 z-0 relative">
      ...
      <nav className="md:hidden fixed bottom-1 left-1 right-1 h-[60px] terminal-glass rounded-xl px-2 !backdrop-blur-xl flex items-center justify-around z-50">
        {navItems.map((item) => {
          const isActive = location.pathname === item.path;
          return (
            <Link
      ...
      <footer className="hidden md:flex h-6 items-center justify-between border-t border-brand-border bg-brand-bg px-4 !text-[10px] uppercase font-mono text-zinc-500 tracking-wider">
