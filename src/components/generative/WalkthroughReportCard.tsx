...
export function WalkthroughReportCard({
  data,
  mediaUrl,
  onApprove,
}: {
  ...
  return (
    <div className="bg-[#09090b] border border-[#27272a] p-4 font-mono text-xs text-zinc-300 w-full rounded">
      ...
      {mediaUrl && (
        <div className="mb-4">
          <p className="text-zinc-500 uppercase tracking-widest text-[9px] mb-2">
            // CAPTURED KEYFRAME
          </p>
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        ...
        <div className="border border-[#27272a] flex flex-col items-center justify-center py-4">
          ...
          <div
            className={`text-xl font-bold ${data.anomalies && data.anomalies.length > 0 ? 'text-red-400' : 'text-emerald-400'}`}
          >
        ...
        <div className="mb-4">
          ...
          <div className="space-y-2">
            {data.anomalies.map((anom, i) => (
              <div
                key={i}
                className="flex flex-col gap-1 bg-[#1a0f0f] border border-red-900/30 p-2 text-red-200/80"
              >
                <div className="flex items-center gap-2">
        ...
        <div className="mb-4">
          ...
          <div className="space-y-2">
            {data.actions_required.map((act, i) => (
              <div
                key={i}
                className="flex items-center justify-between bg-zinc-900 border border-[#27272a] p-2 hover:bg-zinc-800 transition-colors"
              >
          ...
          <div className="border border-[#27272a] bg-[#111] p-3 mb-2 flex items-center justify-between">
            ...
            <span
              className={`text-xl font-bold ${
                (data.analysisResults.overallHealthScore || 0) > 85
                  ? 'text-emerald-400'
                  : (data.analysisResults.overallHealthScore || 0) > 65
                    ? 'text-yellow-400'
                    : 'text-red-400'
              }`}
            >
              ...
              {data.analysisResults.findings.map((finding, idx) => (
                <div key={idx} className="bg-[#09090b] border border-[#27272a] p-3">
                  ...
                  <div className="flex items-center gap-2 text-zinc-500 text-[10px] uppercase tracking-widest mb-2 border-b border-[#27272a] pb-2">
                    ...
                    <span>•</span>
                    <span>
                      {(finding.confidence ? finding.confidence * 100 : 0).toFixed(0)}% CONFIDENCE
                    </span>
                  </div>
