...
export function OcrCaptureCard({
  data,
  mediaUrl,
  onApprove,
}: {
  ...
  return (
    <div className="bg-[#09090b] border border-[#27272a] p-4 font-mono text-xs text-zinc-300 w-full rounded">
      <div className="flex items-center justify-between border-b border-[#27272a] pb-3 mb-4">
        ...
        <span
          className={`text-zinc-500 text-[10px] uppercase font-bold px-2 py-1 border ${approved ? 'border-emerald-500/30 text-emerald-400' : 'border-[#27272a]'}`}
        >
      ...
      <div className="flex gap-4 mb-6">
        {mediaUrl && (
          <div className="w-1/3 border border-[#27272a] p-1 bg-[#1a1a1a] flex items-center justify-center">
            <img
              src={mediaUrl}
              className="w-full h-auto object-cover grayscale opacity-90 mix-blend-screen"
              alt="Sensor crop"
            />
          </div>
        )}
        <div className={`grid grid-cols-2 gap-4 ${mediaUrl ? 'w-2/3' : 'w-full'}`}>
          <div className="border border-[#27272a] p-3 flex flex-col justify-center items-center py-6 relative overflow-hidden">
            <span className="text-zinc-500 uppercase tracking-widest text-[9px] mb-2 z-10">
              Extracted Temp
            </span>
            ...
            {data.target_temperature && (
              <div className="mt-2 text-[10px] text-zinc-500 z-10">
                ...
                <span className="ml-2 text-zinc-600">
                  (Δ{' '}
                  {data.temperature && data.target_temperature
                    ? (data.temperature - data.target_temperature).toFixed(1)
                    : '--'}
                  °)
                </span>
              </div>
          ...
          <div className="border border-[#27272a] p-3 flex flex-col justify-center items-center py-6 relative overflow-hidden">
            ...
            {data.target_humidity && (
              <div className="mt-2 text-[10px] text-zinc-500 z-10">
                ...
                <span className="ml-2 text-zinc-600">
                  (Δ{' '}
                  {data.humidity && data.target_humidity
                    ? (data.humidity - data.target_humidity).toFixed(1)
                    : '--'}
                  %)
                </span>
              </div>
