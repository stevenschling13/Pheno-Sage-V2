...
export function HarvestYieldForecastCard({
  data,
  onApprove,
}: {
  data: HarvestData;
  ...
  const scoreColor =
    data.pheno_score === 'S' || data.pheno_score === 'S-Tier'
  ...
  return (
    <div className="bg-[#09090b] border border-[#27272a] p-4 font-mono text-xs text-zinc-300 w-full rounded">
      <div className="flex items-center justify-between border-b border-[#27272a] pb-3 mb-4">
        ...
        <span
          className={`text-zinc-500 text-[10px] uppercase font-bold px-2 py-1 border ${approved ? 'border-amber-500/30 text-amber-400' : 'border-[#27272a]'}`}
        >
      ...
      <div className="grid grid-cols-3 gap-4 mb-6">
        <div
          className={`col-span-1 border p-4 flex flex-col items-center justify-center ${scoreColor}`}
        >
        ...
        <div className="col-span-2 border border-[#27272a] p-4 flex flex-col justify-center bg-[#111]">
          <span className="text-[10px] text-zinc-500 uppercase tracking-widest mb-2">
            Est. Dry Yield
          </span>
      ...
      <div className="border border-[#27272a] bg-[#09090b] p-3 mb-6">
        ...
        <div className="text-zinc-300 leading-relaxed">
          {data.canopy_evaluation ||
            'Insufficient longitudinal data to perform accurate architectural breakdown.'}
        </div>
      </div>
